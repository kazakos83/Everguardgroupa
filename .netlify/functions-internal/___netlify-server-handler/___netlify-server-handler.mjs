...
} from '/var/task/everguard-live-repo/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/everguard-live-repo/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/everguard-live-repo')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/everguard-live-repo',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/everguard-live-repo/.netlify/dist/run/handlers/server.js')
